import pyqc, pyqc.measure, pyqc.templates
...
if test_expr:
    ...
    i_e = idle_m.efficient_measurements(1200, measure_ent=False, measure_eom=False, measure_GKP=False)
    print(f"Idle circuit expr is {i_e['Expr']}, should be O(10) (4.317 in paper)") #['Expr']
    ...
    a_e = circuit_A_m.efficient_measurements(1200, measure_ent=False, measure_eom=False, measure_GKP=False)
    print(f"Circuit_A expr is {a_e['Expr']}, should be O(0.1) (0.2 in paper)") #['Expr']
    ...
    b_e = circuit_B_m.efficient_measurements(1200, measure_ent=False, measure_eom=False, measure_GKP=False)
    print(f"Circuit_B expr is {b_e['Expr']}, should be O(0.1) - O(0.01) (0.02 in paper)") #['Expr']
    ...
    print(f"Random unitary expr is {rum_expr}, should be O(0.01) - O(0.001) (0.07 in paper)")
    print("Test passed ✔ \n")
...
if test_qg:
    ...
    init_layer: list[pyqc.Gate] = [pyqc.fixed_R_y(i, 4, np.pi / 4) for i in range(4)]
    layer1: list[pyqc.Gate] = [pyqc.R_z(0, 4), pyqc.R_x(1, 4), pyqc.R_y(2, 4), pyqc.R_z(3, 4), pyqc.CHAIN(pyqc.CNOT, 4)]
    layer2: list[pyqc.Gate] = [pyqc.R_x(0, 4), pyqc.R_x(1, 4), pyqc.R_x(2, 4), pyqc.R_y(3, 4), pyqc.CHAIN(pyqc.CNOT, 4)]
    layer3: list[pyqc.Gate] = [pyqc.R_z(0, 4), pyqc.R_x(1, 4), pyqc.R_y(2, 4), pyqc.R_y(3, 4), pyqc.CHAIN(pyqc.CNOT, 4)]
...
if test_haar:
    ...
    for N in range(1, 8):
        ...
        eom = hm.efficient_measurements(2000, measure_expr=False, measure_ent=False, measure_eom=True, measure_GKP=False)["Magic"]
        eoms.append(eom[0])
        ...
        print(f"{N} qubit Haar state EoM = {eom[0]:.5f} +/- {eom[1]:.6f} vs analytic expr = {avg_magic:.6f}")
    print("Test passed ✔ \n")
...
if test_TFIM:
    ...
    assert isclose(F, 1)
    print(f"Overlap between QuTIP groundstate of TFIM Hamiltonian and circuit minimised state is {F:.4f}")
    print("Test passed ✔ \n")


if test_eff_H:
    print("Test finding |H_eff| for an n-qubit hardware efficient circuit - scales as 2**N: ")
    for n in range(2, 9, 2): #2, 9, 2
        ...
        print(f"|H_eff| for generic HE circuit for {n} qubits is {eff_H:.6f}, should be around {2**n}")
        assert isclose(eff_H, 2**n, rel_tol=0.1)
    print("Test finding |H_eff| for an n-qubit, half-filled zfsim circuit - scales as N choose N//2: ")
    for n in range(2, 9, 2): #2, 9, 2
    ...
    print("Test finding |H_eff| for an 8-qubit, K filled zfsim circuit - scales as 8 choose K: ")
    N = 8
    for k in range(1, 5):
        zfsim = pyqc.templates.generate_circuit("zfsim", N, N)
        init_state = [qt.basis(2, 1) for i in range(k)] + [qt.basis(2, 0) for i in range(N-k)]
        ...
        print(f"|H_eff| for zfsim circuit for {k} spin-ups, {N} qubits is {eff_H:.6f}, should be around {n_c_k}")
        assert isclose(eff_H, n_c_k, rel_tol=0.4)
    print("Test passed ✔ \n")
